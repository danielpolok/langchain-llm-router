...
- **Check** — phrased as the assertion a test makes. If it can't be written as one, it isn't a
  requirement yet.
- **Settled rules (D1–D8)** are the decisions the principles left open. They are recorded in
...
`as_dict()` is what rides under `response_metadata["routing"]` and, unchanged, in the router's
chain-run metadata. `routing_decision(message)` reads it back off a response.
...
| **D8** | The decision record is the six-field schema above, and exactly **one** streamed chunk carries it. | `merge_dicts` concatenates strings that repeat across chunks, so a record on every chunk aggregates to `"frontierfrontier…"` (spike surprise 6). |
...
| ID | Requirement | Check | Task |
| --- | --- | --- | --- |
| REQ-C5-1 | The router's own run is a **chain** run whose metadata carries the decision; the selected route's call is the only LLM run, nested inside it. | Offline tracer records exactly one `on_chat_model_start`; its parent run is the router's chain run, whose metadata contains the record. | T-117 |
...
| REQ-R2-2 | The same record is on the router's run. | Visible in the offline tracer's chain-run metadata and in a live LangSmith trace. | T-114 |
...
| REQ-R3-2 | An opt-in strategy's own calls are costed to the strategy's model, never folded into the router or the answering route. | The classifier / embedding run appears as its own run with its own usage; the answering route's usage is unchanged. | T-117 |
...
| R6 | REQ-R6-1…4 | T-111, T-132, T-151 |
