"""Experimentation (PRD §4): force a route per call to compare models on the same traffic.

Runtime config pins one call to a named route (C4, R11, D7) — the strategy is skipped
entirely (D2), and a forced route is never silently swapped: an unknown or tool-incapable
forced route raises `ForcedRouteError` by default, and only falls back when the router is built
with `on_unavailable_forced_route="fallback"`. See `docs/decision-record.md` for the full R9/R11
...
def main() -> list[AIMessage]:
    ...
    # Forcing an unknown route errors by default (R11) rather than silently falling back —
