"""An offline stand-in for what LangSmith would charge for a trace (T-004).
...
These helpers walk collected runs the same way, so the spike can score designs without an API
key. The real check against a LangSmith trace still needs one.
"""
